...
class Events(commands.Cog):
    ...
    async def _send_event_list(self, channelId: int, interval: int, noun: str, now: datetime):
        ...
        if len(allEvents) > 0:
            ...
            response = (
                self.bot.get_cog("cogs.NaturalLanguage").prompt("Given the list of events provided, generate a small list of upcoming events ordered by date and include a short description. Here is the list of events:" + events) 
                or 
                string + urls
            )
