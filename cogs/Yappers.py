...
class Yappers(commands.Cog):
            ...
            for x in range(0, len(self.topYappers[message.guild.id]) - 1):
                if self.topYappers[message.guild.id][x]["user_id"] != newTopYappers[x]["user_id"]:
                    await message.reply(
                        ...
                        f"-# This message will self-destruct in <t:{int((datetime.now(timezone.utc) + timedelta(minutes=1)).timestamp())}:R>",
                        delete_after = 60
                    )
                    break
    ...
    @group.command(name = "subscribe", description = "Subscribe this guild to top yapper announcements.")
    ...
    @group.command(name = "leaderboard", description = "List the top yappers in this server.")
